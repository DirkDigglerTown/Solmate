...
// VRM Animation & TTS Fix for Solmate
// Add this to the end of your script.js to fix movement and audio

// ===== TTS & ANIMATION COMPREHENSIVE FIX =====
log('🎭 FIXING VRM ANIMATIONS AND TTS SYSTEM...');

// 1. Enhanced Audio System with Multiple Fallbacks
class EnhancedTTSSystem {
  ...
  async setupAudioSystem() {
      ...
      if (!this.isAudioEnabled) {
        try {
          ...
          if (this.speechSynthesis) {
            ...
            log(`🔊 Audio enabled! Available voices: ${voices.length}`);
          }
          
          this.isAudioEnabled = true;
          log('✅ Enhanced TTS system ready!');
    ...
    // Also enable when voices are loaded
    if (this.speechSynthesis) {
  ...
  showAudioReady() {
    ...
    notification.onclick = () => {
      this.speak("Hello! I'm Solmate, your Solana companion. Audio is working perfectly!");
      notification.remove();
    };
  ...
  async speak(text, voice = 'default') {
    ...
    log(`🗣️ TTS Request: "${text.substring(0, 30)}..."`);
  ...
  async playAudioBlob(blob, text) {
    try {
      ...
      log('✅ API TTS playing successfully');
      
    } catch (err) {
      log('Audio blob playback failed:', err);
      this.speakWithBrowser(text);
    }
  }
  ...
  stopLipSync() {
    ...
    if (window.vrmAnimator) {
      window.vrmAnimator.stopTalking();
    }
    
    // VRM expression
    if (currentVRM && currentVRM.expressionManager) {
      try {
        currentVRM.expressionManager.setValue('aa', 0); // Close mouth
...
// 2. Advanced VRM Animation System
class VRMAnimator {
  constructor() {
    this.vrmModel = null;
  ...
  fixTPose() {
    ...
    this.vrmModel.traverse((child) => {
      if (child.isBone || child.type === 'Bone') {
        ...
        } else if (boneName.includes('lowerarm') || boneName.includes('forearm')) {
          child.rotation.set(0, 0, 0);
        ...
        if (boneName.includes('leftshoulder') || boneName.includes('leftupperarm')) {
          child.rotation.z = -0.2; // Lower left arm
        } else if (boneName.includes('rightshoulder') || boneName.includes('rightupperarm')) {
          child.rotation.z = 0.2; // Lower right arm
        }
      }
    });
  ...
  setupAnimationMixer() {
    ...
    this.animationMixer = new THREE.AnimationMixer(this.vrmModel);
    
    // If there are built-in animations, use them
    if (mixer && mixer.getRoot() === this.vrmModel) {
      this.animationMixer = mixer;
      log('Using existing animation mixer');
    } else {
      // Replace global mixer
      mixer = this.animationMixer;
      log('Created new animation mixer');
    }
  }
  ...
  animateHead() {
    ...
    if (headBone) {
      // Random head movements every few seconds
...
// 4. Override existing TTS functions
window.queueTTS = function(text, voice = 'nova') {
  enhancedTTS.queue(text, voice);
};

window.clearAudioQueue = function() {
  enhancedTTS.clear();
};

// 5. Enhanced sendMessage function with better TTS
...
// 7. Auto-setup when VRM loads
const originalProcessAndAddVRM = window.processAndAddVRM;
window.processAndAddVRM = async function(gltf) {
  // Call original processing
  if (originalProcessAndAddVRM) {
    await originalProcessAndAddVRM(gltf);
  }
  
  // Setup animations after VRM loads
  setTimeout(() => {
    vrmAnimator.setupVRM(gltf.scene);
    
    // Welcome message with animation
    setTimeout(() => {
      vrmAnimator.setExpression('happy', 0.8, 3000);
      enhancedTTS.speak("Hello! I'm Solmate, your enhanced Solana companion. Click anywhere to enable my voice!");
    }, 2000);
  }, 1000);
};

log('🎭✅ VRM ANIMATION & TTS SYSTEM LOADED!');
console.log('🎤 TTS Commands:');
console.log('  - testTTS() - Test voice system');
console.log('  - enhancedTTS.speak("text") - Make Solmate speak');
console.log('  - enhancedTTS.clear() - Clear audio queue');
console.log('🎭 Animation Commands:');
console.log('  - testAnimations() - Test facial expressions');
console.log('  - fixTPose() - Fix T-pose manually');
console.log('  - startTalking() - Test talking animation');
console.log('  - vrmAnimator.setExpression("happy", 1) - Set expression');

// Auto-test after 3 seconds
setTimeout(() => {
  const vrmModel = scene?.getObjectByName('VRM_Model');
  if (vrmModel) {
    log('🎯 Auto-applying animation fixes...');
    vrmAnimator.setupVRM(vrmModel);
  }
}, 3000);
